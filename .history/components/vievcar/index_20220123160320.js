...
const Carviewslider = ({spantext}) => {
    ...
    function setcrashed (){
        var crashedspan = document.getElementsByClassName("setstarge");
        for(var i = 0; i < crashedspan.length; i++){
            if(crashedspan[i].getAttribute("data-name")=="crashed"){
                crashedspan[i].textContent = "Есть повреждения"
            }else{
                crashedspan[i].textContent = "Нет повреждений"
            }
        }
    }
    return (
            ...
            <div className='carviewslider_top'>
                <Swiper onSwiper={setThumbsSwiper} loop={true} spaceBetween={10} slidesPerView={4} freeMode={true} watchSlidesProgress={true} className="mySwiper">
                    <SwiperSlide>
                        ...
                        <span className='setstarge' data-name="crashed">Есть повреждения</span>
                    </SwiperSlide>
                    <SwiperSlide>
                        <button>Левая сторона</button>
                        <span className='setstarge'>Нет повреждений</span>
                    </SwiperSlide>
                    <SwiperSlide>
