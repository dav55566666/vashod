...
class Startpage extends React.Component {
    ...
    componentDidMount() {
      axios.get(`https://raw.githubusercontent.com/dav55566666/test/master/myapi5.json`)
        .then(res => {
                ...
                <div className='startpage_logined_cont_top'>
                  ...
                  <Select defaultValue="Поиск" style={{ width: 120 }} onChange={handleChange}>
                    {[this.state.posts].map(post =>
                      <Option value={post.car.carname}><span className='selectedchek'></span>{post.car.carname}</Option>
                    )}
                  </Select>
                </div>
