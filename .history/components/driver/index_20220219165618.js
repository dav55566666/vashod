import React, { useState,Component } from 'react';
import ReactDOM from 'react-dom';
import axios from "axios";
import Input from "../../uikit/input"
import NavMenu from "../../uikit/nav"
import A from "../../uikit/button/atag"
import 'antd/dist/antd.css';
import { Select } from 'antd';
const { Option } = Select;
import $ from "jquery";
import api from '../../uikit/api'
class DriverPage extends React.Component {
  state = {
    posts: []
  }

  componentDidMount() {
    axios.get(api.driverapi)
      .then(res => {
        const posts = res.data.driver;
        this.setState({posts});
      });
  }
  render(){
      let selectedcar2 = 0;
      function handleChange2(value2) {
        selectedcar2=`${value2}`
        document.querySelector("#driversname h4").textContent= selectedcar2
        if(selectedcar2!="Поиск"){
          document.querySelector("#driversname").style.display= "block"
          document.getElementById("senddriv22").classList.remove("disabled")
        }else{
          document.querySelector("#driversname").style.display= "none";
          document.querySelector("#senddriv22").classList.add("disabled")
        }
      }
      function sendform(){
        document.getElementById("username").textContent = document.querySelector("#driversname h4").textContent;
        document.querySelector(".workingpage").style.display = "block";
        document.querySelector(".driver").style.display = "none";
      }
      return (
        <div className="startpage startpagedriver">
            <div className='startpage_logined'>
              <NavMenu setactivlink={""}/>
              <form action='working'>
                <div className='startpage_logined_cont'>
                  <div className='startpage_logined_cont_top'>
                    <p>Выберите водителя из списка</p>
                    <Select defaultValue="Поиск" style={{ width: 120 }} onChange={handleChange2}>
                      {this.state.posts.map((post, index) =>
                        <Option key={index} value={post.drivername}><span className='selectedchek'></span>{post.drivername}</Option>
                      )}
                    </Select>
                  </div>
                  <div className='startpage_logined_cont_bottom' >
                    <div className='selectedcar' id='driversname'>
                      <p>Выбран водитель</p>
                      <h4></h4>
                    </div>
                    <div className='buttonnext'>
                      <Input value={"ДАЛЕЕ"} className={"submit disabled buttonlink"} type={"button"} id={"senddriv22"} myclick={sendform} />
                    </div>
                  </div>
                </div>
              </form>
            </div>
        </div>
      );
  }
}

export default DriverPage;
