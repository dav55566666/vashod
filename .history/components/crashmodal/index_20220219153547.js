    ...
    const fileChange = (event) => {
     ...
     function sendcrash(){
        ...
        if(myid!=null){
            ...
            thiscrash.textContent = document.getElementById("opis").value
            thiscrash.setAttribute("data-name", document.getElementById("name").value)
            document.getElementById("crashname").textContent = document.getElementById("name").value
            document.getElementById("crashinfo").textContent = document.getElementById("opis").value
        ...
        var crashimg = document.querySelectorAll('.imgswiper .swiper-slide img');
        var crashsrc = []
        for(var imgsrc = 0; imgsrc < crashimg.length; imgsrc++){
            crashsrc.push(crashimg[imgsrc].attributes.src.nodeValue)
        }
        $.ajax({
            type:"POST",
        ...
        <div className="addcrashmodal">
            <div className='addcrashmodal_cont'>
                ...
                <form>
                    ...
                    <label className="textinp" > Введите название поврежденной детали
                    ...
                    <label className="subinp">
                        <input type="button" value="Сохранить" onClick={sendcrash} />
                    </label>
                </form>
            </div>
        </div>
