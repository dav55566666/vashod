...
class Addcrash extends React.Component {
    ...
    componentDidMount() {
    axios.get(`https://raw.githubusercontent.com/dav55566666/test/master/myapi6.json`)
        .then(res => {
                    ...
                    <div className='addcrash_left_box'>
                        <div className='crashdetal'>
                            {this.state.posts.map((post, index) =>
                                <button key={index} value={post.carash.crashname} data-src={post.carash.img[post]} onClick={(e) => setcrash (e)} className='crashedbtn'>{post.carash.crashname}</button>
                            )}
                        </div>
