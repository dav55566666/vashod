...
class Addcrash extends React.Component {
    ...
    componentDidMount() {
    axios.get(`https://raw.githubusercontent.com/dav55566666/test/master/myapi7.json`)
        .then(res => {
    ...
    render(){
        ...
        function setcrash(e){
            ...
            var arraymy2 = [];
            var slider = document.querySelectorAll(".imgswiper2 .swiper-slide img") 
            arraymy2 = arraymy.split(",")
            console.log(slider)
            for(var g = 0; g < arraymy2.length; g++){
                slider[g].setAttribute("src", arraymy2[g])
            }
            document.querySelector(".addcrash_right").style.display = "block"
            document.querySelector(".addcrash_right .addcrash_right_title h4").textContent(e.target.getAttribute("data-name"))
            document.querySelector(".addcrash_right .crashedinfo p").textContent(e.target.value)
        }
        return (
                    ...
                    <div className='addcrash_left_box'>
                        <div className='crashdetal'>
                            {this.state.posts.map((post, index) =>
                                <button key={index} value={post.carash.crashname} data-src={post.carash.img} data-name={post.carash.crashinfo} onClick={(e) => setcrash (e)} className='crashedbtn'>{post.carash.crashname}</button>
                            )}
                        </div>
                ...
                <div className='addcrash_right'>
                    <div className='addcrash_right_box'>
                        ...
                        <div className='crashedinfo'>
                            <p>Описание повреждения</p>
                        </div>
                        <div className='crashedimg'>
                        <Swiper slidesPerView={"auto"} spaceBetween={34} pagination={{"clickable": true}} className="imgswiper2">
                            <SwiperSlide>
                                <img src="" />
                            </SwiperSlide>
                            <SwiperSlide>
