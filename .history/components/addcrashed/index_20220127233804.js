...
class Addcrash extends React.Component {
    ...
    componentDidMount() {
    axios.get(`https://raw.githubusercontent.com/dav55566666/test/master/myapi.json`)
        .then(res => {
                    ...
                    <div className='addcrash_left_box'>
                        <div className='crashdetal'>
                            {this.state.posts.map((post, index) =>
                                <button key={index} value={post.carash.crashname} onClick={(e) => setcrash (e)} className='crashedbtn'>{post.carash.crashname}</button>
                            )}
                            {this.state.posts.carash.img.map((post2, index2) =>
                                <img key={index2} src={post2.crashimg} className='crashedbtn' />
                            )}
                        </div>
